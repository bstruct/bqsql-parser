...
fn parse_tokens(bqsql: &str) -> Vec<BqsqlDocumentToken> {
    ...
    for line in bqsql.lines() {
        ...
        split1.push(line.len());

        for start in split1 {
            if let Some(previous) = previous_start {
                let partial = line[previous..start].to_string();
...
fn parse_tokens_single_line_operation() {
    ...
    assert_eq!("--test, another `table` 123 \"back\" to 'dust'", result[4].token);
...
fn parse_tokens_single_line_string() {
    let result = parse_tokens("SELECT \"this is a ''' string \"--test, another `table` 123 \"back\" to 'dust'");
